...
export default function TherapistDash(props) {
  ...
  useEffect(() => {
    // sinkroniziranje svega za reschedule ovisno o odabranom sessionu
  ...
  if (mySchedule[selectedWeek] != null) {
    // mapiranje podataka iz baze na jsx (html) elemente za ispis
    scheduleElements = mySchedule[selectedWeek]
      .sort(
        (s1, s2) =>
          new Date(s1.dateTime).getTime() - new Date(s2.dateTime).getTime()
      )
      .map((session) => {
        // kartice sesija u rasporedu
        ...
        return (
          <div
            className={`${cardClass} ${
              id == selectedSession.id ? s.session_selected : ""
            }`}
            key={id}
            onClick={() => {
          ...
          >
            ...
            <h3 className={s.session_time}>
              {new Date(dateTime).getHours()}:00 -{" "}
              {new Date(dateTime).getHours() + 1}:00
            </h3>
    ...
    scheduleElements = (
      <p className={s.no_sessions}>
        {t("therapistDash.schedule.noSessionsThisWeek")}
      </p>
    );
  }
  ...
  function handleNotesEdit(action) {
    if (action == "cancel") {
      ...
      setMySchedule((prevSchedule) => ({
        ...prevSchedule,
        [selectedWeek]: [
          ...prevSchedule[selectedWeek].filter(
            (session) => session.dateTime != selectedSession.dateTime
          ),
          {
            ...selectedSession,
            sessionNotes: notesInput,
      ...
      axios({
        url:
          "https://medbay-backend-4957d331fef0.herokuapp.com/api/appointment/session-notes/" +
  ...
  return (
    <>
      <div
        className={`${s.therapist_dash_main} ${
          patientPopup && s.covered_by_popup
        }`}
      >
        <div className={s.container_left}>
          <h2 className={s.container_title}>
            {t("therapistDash.schedule.title")}
          </h2>

          <div className={s.date_wrapper}>
            <span className={s.date_arrow} onClick={goBackWeek}>
              &#60;
            </span>
            <p className={s.container_date}>{formatWeek(selectedWeek)}</p>
            <span className={s.date_arrow} onClick={goForwardWeek}>
              &#62;
            </span>
          </div>
          <img
            src={refresh}
            className={s.date_reset}
            onClick={() => setSelectedWeek(getWeekFirst(new Date()))}
          />

          <div className={s.scroll_container}>
            <div className={s.schedule_card}>{scheduleElements}</div>
          </div>
        </div>

        <div className={s.container_right}>
          <h2 className={s.container_title}>
            {selectedSession.dateTime == nextSession.dateTime
              ? t("therapistDash.schedule.nextSession")
              : t("therapistDash.schedule.selectedSession")}
          </h2>
          ...
          {selectedSession.dateTime == "--" ? (
            <>
              ...
              <div
                className={`${s.selected_session} ${s.no_sessions_container}`}
              >
                <p className={s.no_sessions}>
                  {t("therapistDash.schedule.noUpcomingSessions")}
                </p>
              </div>
            </>
          ) : (
            <>
              <p className={s.container_date}>
                {formatDate(new Date(selectedSession.dateTime))}
              </p>
              ...
              <div className={s.right_cards}>
                <div className={s.selected_session}>
                  <div className={s.session_info}>
                    ...
                    <div className={s.info_values}>
                      <p>
                        {new Date(selectedSession.dateTime).getHours()}:00 -{" "}
                        {new Date(selectedSession.dateTime).getHours() + 1}:00
                      </p>
                    </div>
                    ...
                    <div className={s.info_values}>
                      <p>
                        {selectedSession.numberOfSessionsCompleted}/
                        {selectedSession.numberOfSessions}
                      </p>
                    </div>
                    ...
                    <div className={s.info_values}>
                      <p
                        className={s.patient_link}
                        onClick={() => setPatientPopup(true)}
                      >
                        {selectedSession.patient.firstName +
                          " " +
                          selectedSession.patient.lastName}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className={s.session_notes}>
                  <div className={s.notes_header}>
                    <p className={s.note_title}>
                      {t("therapistDash.notes.title")}
                    </p>

                    {editingNotes ? (
                      <button
                        className={s.note_cancel}
                        onClick={() => handleNotesEdit("cancel")}
                      >
                        {t("therapistDash.notes.cancel")}
                      </button>
                    ) : (
                      <p
                        className={`${s.add_note_text} ${
                          notesDisabled && s.disabled_text
                        }`}
                      >
                    ...
                    <button
                      className={`${s.note_edit} ${
                        notesDisabled && s.button_disabled
                      }`}
                      onClick={() =>
                        notesDisabled ? {} : handleNotesEdit("edit")
                      }
                    >
                      {editingNotes
                        ? t("therapistDash.notes.save")
                        : selectedSession.sessionNotes == ""
                        ? t("therapistDash.notes.add")
                        : t("therapistDash.notes.edit")}
                    </button>
                  </div>
                  {editingNotes ? (
                    <textarea
                      ...
                      className={s.note_box}
                      type="text"
                      onChange={(event) => setNotesInput(event.target.value)}
                      placeholder={t("therapistDash.notes.placeholder")}
                      name="note"
                      value={notesInput}
                    />
                  ) : (
                    <div className={s.note_box}>
                      <p className={s.note_contents}>
                        {selectedSession.sessionNotes}
                      </p>
                    </div>
                  )}
