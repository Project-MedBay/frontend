        ...
        <div className={s.popupContainer}>
            <div className={s.popup} style={{ width: type === "therapy" ? '60vw' : 'default'}}>
                ...
                <h2 className={s.requestNumerator}>
                    {type == "registration" ? t('adminVerificationPopup.registration') : t('adminVerificationPopup.therapy')} {t('adminVerificationPopup.requestNo')}{popupData.request_id}
                </h2>
                {
                    type === 'registration' ? (
                        <div className={s.userDataFrame}>
                            <h3 className={s.userDataTitle}>{t('adminVerificationPopup.userData')}</h3>
                            <div className={s.keyValuePairs}>
                                ...
                                <div className={s.keyValuePair}>
                                    <p className={s.key}>{t('adminVerificationPopup.phoneNumber')}</p>
                            ...
                            <div className={s.therapyInfo}>
                                ...
                                <div className={s.keyValuePairs}>
                                    <div className={s.keyValuePair}>
                                        <p className={s.key}>{t('adminVerificationPopup.therapyID')}</p>
                                        <p className={s.value}>{popupData.therapy_id}</p>
                                    </div>
                                    <div className={s.keyValuePair}>
                                        <p className={s.key}>{t('adminVerificationPopup.duration')}</p>
                                        <p className={s.value}>{popupData.duration}</p>
                                    </div>
                                    <div className={s.keyValuePair}>
                                        <p className={s.key}>{t('adminVerificationPopup.noOfSessions')}</p>
                                        <p className={s.value}>{popupData.number_of_sessions}</p>
                                    </div>
                                </div>
                            </div>
                            <div className={s.sessionTab}  style={{ height: isExpanded ? 'auto' : '30vh' }}>
                                <h3 className={s.userDataTitle}>{t('adminVerificationPopup.sessions')}</h3>
                                <div className={s.sessionList}>
                                    {sessionElements}
                                </div>
                                <button className={s.expandBtn} onClick={toggleExpand}>
                                        {isExpanded ? t('adminVerificationPopup.collapse') : t('adminVerificationPopup.expand')}
                                </button>
                            </div>
                        </div>
                    )}
                ...
                {action ? (
                    ...
                    <div className={s.buttons}>
                        <button className={s.approveBtn} onClick={() => handleActionClick('approve')}>{t('adminVerificationPopup.buttonApprove')}</button>
                        <button className={s.rejectBtn} onClick={() => handleActionClick('reject')}>{t('adminVerificationPopup.buttonReject')}</button>
                    </div>
                )}
            </div>
        </div>
