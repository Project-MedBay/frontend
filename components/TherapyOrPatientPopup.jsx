...
export default function TherapyOrPatientPopup(props) {
  ...
  for (let row of rows) {
    ...
    let rowElements = row.map((item) => {
      ...
      return (
        <p className={s.popup_info} key={item}>
          {t("therapyOrPatientPopup.infoLabels." + item)}: <span>{value}</span>
        </p>
      );
    });
    infoElements.push(
      <div className={rowClass} key={infoElements.length}>
        {rowElements}
      </div>
    );
  }
  const sessionElements = popupSessions
    ?.sort(
      (s1, s2) =>
        new Date(s1.appointmentDate).getTime() -
        new Date(s2.appointmentDate).getTime()
    )
    .map((session) => {
      ...
      let sessionInfo = (
        <>
          <p className={s.session_datetime}>
            {formatDate(new Date(session.appointmentDate))}{" "}
            {t("therapyOrPatientPopup.atSessionInfo")}{" "}
            {new Date(session.appointmentDate).getHours()}:00
          </p>
          {session.sessionNotes != "" ? (
            <p
              className={`${s.session_notes} ${s.notes_link}`}
              onClick={() => viewNote(session)}
            >
              {viewingNotesOf.appointmentId === session.appointmentId
                ? t("therapyOrPatientPopup.session.collapse")
                : t("therapyOrPatientPopup.session.viewNotes")}
            </p>
          ) : popupType == "therapy" || !session.show || !sessionPassed ? (
            <p className={s.session_notes}>
              {t("therapyOrPatientPopup.session.noNotes")}
            </p>
          ) : (
            <button
              className={s.note_edit}
              onClick={() => handleNotesEdit("add", session)}
            >
              {t("therapyOrPatientPopup.session.addNote")}
            </button>
          )}
        </>
      );
      return (
        <div
          className={`${s.session_card} ${sessionPassed && s.session_passed}`}
          key={session.appointmentId}
        >
          <div className={s.session_info}>
            {popupType == "patient" ? (
              <>
                <p className={s.session_therapy}>
                  {session.therapyName.toUpperCase()}
                </p>
          ...
          {viewingNotesOf.appointmentId === session.appointmentId && (
            <>
              <div className={s.box_container}>
                ...
                {editingNotes ? (
                  <textarea
                    ...
                    placeholder={t("therapyOrPatientPopup.session.placeholder")}
                    name="note"
                    value={notesInput}
                  />
                ) : (
              ...
              {popupType == "patient" && session.show && sessionPassed && (
                <div className={s.buttons_container}>
                  {editingNotes && (
                    ...
                    >
                      {t("therapyOrPatientPopup.session.cancel")}
                    </button>
                  )}

                  <button
                    className={s.note_edit}
                    onClick={() => handleNotesEdit("save")}
                  >
                    {editingNotes
                      ? t("therapyOrPatientPopup.session.saveNote")
                      : t("therapyOrPatientPopup.session.editNote")}
                  </button>
                </div>
              )}
            </>
        ...
        .then((res) =>
          setPopupSessions(
            popupSessions.map((session) => {
              if (session.appointmentId == viewingNotesOf.appointmentId)
                return {
                  ...session,
                  sessionNotes: notesInput,
                };
  ...
  return (
    <div
      className={`${s.popup} ${
        popupType == "patient" && s.popup_wide
      } ${darkModeClass}`}
    >
      <div className={s.popup_header}>
        <h1 className={s.popup_title}>
          {title.toUpperCase()}
          <span>{` (#${popupType == "patient" ? "User" : ""}${
            popupData.id
          })`}</span>{" "}
          {/* NOTE vidit jel ovo triba minjat za therapy */}
        </h1>
      ...
      {popupType == "patient" ? (
        <div className={s.popup_main}>
          ...
          <div className={s.main_info}>{infoElements}</div>
        </div>
      ) : (
        infoElements
      )}
      ...
      <h2 className={s.popup_sessions}>
        {popupSessions.length
          ? `${popupSessions.length} ` +
            t("therapyOrPatientPopup.popup.sessionCount")
          : t("therapyOrPatientPopup.noSessionsYet")}
      </h2>
      <div
        className={`${s.sessions_container} ${
          popupType == "patient" && s.popup_tall
        }`}
      >
        {sessionElements}
      </div>
