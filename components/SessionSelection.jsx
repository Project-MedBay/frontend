...
export default function SessionSelection(props) {
  ...
  const [viewingSession, setViewingSession] = useState(
    reschedule ? selectedSessions[0] : null
  );
  ...
  //medbay-backend-4957d331fef0.herokuapp.com

  https: useEffect(() => {
    if (reschedule) {
      axios({
        url:
          "https://medbay-backend-4957d331fef0.herokuapp.com/api/appointment/reschedule/" +
      ...
      axios({
        url:
          "https://medbay-backend-4957d331fef0.herokuapp.com/api/appointment/availability?days=" +
          numberOfDays +
          "&therapyCode=%23" +
          therapyCode.slice(1),
        method: "GET",
  ...
  for (let dateKey in availableSessions) {
    let date = new Date(
      dateKey.slice(6) + "-" + dateKey.slice(3, 5) + "-" + dateKey.slice(0, 2)
    );
    if (date.getTime() > new Date().getTime() + 29 * 60 * 60 * 1000)
      weekDates.push(date);
  }

  var weekElements = weekDates.map((weekDate) => {
    ...
    if (Object.keys(formattedSessions).includes(formatDate(weekDate))) {
      ...
      availableSessions == "" ||
      availableSessions[formatFullDate(weekDate)]?.length == 0 ||
      (!reschedule &&
        (weekDate >= therapyMaxDates.latest ||
          weekDate <= therapyMaxDates.earliest)) ||
      availableSessions[formatFullDate(weekDate)]?.every((time) =>
        blockedSessions[formatDate(weekDate)]?.includes(time)
      )
    ) {
      // ako je u blocked sessions i tamo su svih 12h radnog vrimena
    ...
    return (
      <h3 className={h3Class} key={weekDate.getTime()}>
        {viewingSession != null &&
        formatDate(weekDate) == formatDate(viewingSession)
          ? ">> "
          : ""}
        <span onClick={h3OnClick}>{formatDate(weekDate)}</span>
        {Object.keys(formattedSessions).includes(formatDate(weekDate)) &&
        !reschedule ? (
          <img
            src={x_icon}
            className={s.session_deselect}
            onClick={() => {
              setSelectedSessions((prevArray) =>
                prevArray.filter(
                  (session) => formatDate(session) != formatDate(weekDate)
                )
              );
            }}
          />
  ...
  for (let time = 8; time < 20; time++) {
    ...
    if (viewingSession == null) {
      ...
      timeElements.push(
        <h3 className={h3Class} key={25}>
          {t("sessionSelection.selectDateMessage1")}
          <br />
          {t("sessionSelection.selectDateMessage2")}
        </h3>
      );
      break;
    } else if (
      ...
      availableSessions[formatFullDate(viewingSession)]?.includes(time) ==
        false ||
      ...
      if (reschedule) {
        ...
        h3OnClick = () => {
          setSelectedSessions((prevArray) =>
            [
              ...prevArray.filter(
                (session) => formatDate(session) != formatDate(viewingSession)
              ),
              newDate,
    ...
    timeElements[time - 8] = (
      <h3 className={h3Class} onClick={h3OnClick} key={time * 5}>
        {time}:00 - {time + 1}:00
      </h3>
    );
  }
    ...
    if (!reschedule) {
      ...
      if (selectedSessions.length != 0) {
        let earliest = new Date(
          ...
          selectedSessions[selectedSessions.length - 1].getDate() -
            numOfSessions * 5
        );
        let latest = new Date(selectedSessions[0]).setDate(
          selectedSessions[0].getDate() + numOfSessions * 5
        );
        setTherapyMaxDates({
  ...
  function updateBlockedSessions(date, newBlockedSessions) {
    ...
    for (let i = -1; i < 2; i = i + 2) {
      ...
      while (blockedDate.getDate() != date.getDate()) {
        if (
          blockedDate.getDay() != 0 &&
          blockedDate.getDay() != 6 &&
          blockedDate.getHours() >= 8 &&
          blockedDate.getHours() <= 19
        ) {
          blockedTimes.push(blockedDate.getHours());
        }
        if (
          (blockedDate.getHours() == 1 || blockedDate.getHours() == 23) &&
          blockedTimes.length != 0
        ) {
          let tempArray = newBlockedSessions[formatDate(blockedDate)]
            ? [
                ...new Set([
                  ...newBlockedSessions[formatDate(blockedDate)],
                  ...blockedTimes,
                ]),
              ]
            : [...blockedTimes];
  ...
  return (
    <div className={`${s.selection_main} ${darkModeClass}`}>
      <div className={s.scroll_container}>
        <div
          className={`${s.dates_container} ${!reschedule && s.container_small}`}
        >
          {" "}
          {/* s ovim mozemo minjat visinu containera i velicinu fonta npr za new therapy */}
          {weekElements}
        </div>
      </div>
      ...
      <div className={s.scroll_container}>
        <div
          className={`${s.times_container} ${!reschedule && s.container_small}`}
        >
          {timeElements}
        </div>
      </div>
