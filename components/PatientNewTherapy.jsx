...
export default function PatientNewTherapy(props) {
  ...
  const tooltips = {
    referral:
      t("patientNewTherapy.referralDescription1") +
      "\n" +
      t("patientNewTherapy.referralDescription2") +
      "\n" +
      t("patientNewTherapy.referralDescription3"),
    doctor:
      t("patientNewTherapy.doctorIdDescription1") +
      "\n" +
  ...
  useEffect(() => {
    axios({
      url: "https://medbay-backend-4957d331fef0.herokuapp.com/api/therapyType",
      method: "GET",
      headers: {
  ...
  const duration = selectedSessions.length
    ? Math.floor(
        (new Date(selectedSessions[selectedSessions.length - 1]).setHours(0) -
          new Date(selectedSessions[0]).setHours(0) +
          1000 * 60 * 60) / // offset zbog pomicanja sata
          1000 /
          60 /
          60 /
          24
  ...
  var nextDisabled = () => {
    switch (
      progress // ovdi uvjete za nastavit dalje u svakom koraku
    ) {
      case 1:
        return !codeList.includes(selectedTherapy?.therapyCode);
      case 2:
        return (
          selectedSessions.length != numOfSessions ||
          duration > numOfSessions * 5
        );
      case 3:
        return (
          !finishAgreement ||
          verificationData.referral == "" ||
          verificationData.hlkid == ""
        );
    }
  };
  ...
  const therapyElements = therapies
    .filter((therapy) =>
      t("bodyPartSelection." + selectedBodypart) == t("bodyPartSelection.any")
        ? true
        : t("bodyPartSelection." + therapy.bodyPart) ==
          t("bodyPartSelection." + selectedBodypart)
    )
    .filter((therapy) => {
      for (let term of searchInput.trim().split(" ")) {
        if (therapy.name.toLowerCase().includes(term.toLowerCase()))
          return true;
      }
    })
    .map((therapy) => (
      <div
        className={s.therapy_wrapper}
        key={therapy.therapyCode}
        onClick={() => {
          setSelectedTherapy(therapy);
          setCodeInput(therapy.therapyCode);
        }}
        ref={
          therapy.therapyCode == selectedTherapy.therapyCode
            ? selectedTherapyRef
            : null
        }
      >
        <div className={s.custom_checkbox}>
          <div
            className={`${s.checkbox_fill} ${
              selectedTherapy?.therapyCode == therapy.therapyCode &&
              s.checkbox_selected
            }`}
          ></div>
        </div>
  ...
  function handleCodeInput(event) {
    ...
    if (codeList.includes(event.target.value.toUpperCase())) {
      setSelectedTherapy(
        therapies[codeList.indexOf(event.target.value.toUpperCase())]
      );
  ...
  const sessionElements = selectedSessions.map((session) => (
    <div className={s.session_card}>
      ...
      <p className={s.session_time}>
        {session.getHours()}:00 - {session.getHours() + 1}:00
      </p>
    </div>
  ));
  ...
  function handleFinish() {
    axios({
      url: "https://medbay-backend-4957d331fef0.herokuapp.com/api/therapy/create",
      method: "POST",
      headers: {
        Authorization: `Bearer ${userToken}`, // korisnikov access token potreban za dohvacanje podataka iz baze
      },
  ...
  return (
    <>
      <div
        className={`${s.patient_therapy_main} ${darkModeClass} ${
          successPopup && s.covered_by_popup
        }`}
      >
        ...
        <div className={s.create_container}>
          <div className={s.create_wrapper}>
            {progress == 1 && (
              <>
                <div className={s.therapy_header}>
                  <h2 className={s.header_step}>
                    {t("patientNewTherapy.step1Title")}
                  </h2>
                  <div className={s.header_input}>
                    <p className={s.header_prompt}>
                      {t("patientNewTherapy.step1Subtitle")}
                    </p>
                    <input
                      className={`${s.code_input} ${
                        nextDisabled() && codeInput != "" && s.invalid_input
                      }`}
                      type="text"
                      onChange={handleCodeInput}
                      placeholder={t("patientNewTherapy.codePlaceholder")}
                      name="therapyCode"
                      value={codeInput}
                      autoComplete="off"
                    />
                    <p
                      className={`${s.invalid_text} ${
                        nextDisabled() && codeInput != "" && s.invalid_input
                      }`}
                    >
                      {t("patientNewTherapy.invalidCode")}
                    </p>
                  </div>
                </div>
                ...
                <div className={s.therapy_select}>
                  ...
                  <form
                    className={s.therapy_form}
                    autoComplete="off"
                    onSubmit={(event) => event.preventDefault()}
                  >
                    <h2 className={s.form_title}>
                      {t("patientNewTherapy.filterBy")}{" "}
                      <span>
                        {t(
                          "bodyPartSelection." + selectedBodypart
                        ).toUpperCase()}
                      </span>
                    </h2>
                    <p className={s.form_tip}>
                      {t("patientNewTherapy.searchText")}
                    </p>
                    <input
                      className={s.form_search}
                      type="text"
                      onChange={(event) => setSearchInput(event.target.value)}
                      placeholder={t("patientNewTherapy.searchPlaceholder")}
                      name="search"
                      value={searchInput}
                    />
            ...
            {progress == 2 && (
              <>
                <div className={s.sessions_header}>
                  <h2 className={s.header_step}>
                    {t("patientNewTherapy.step2Title")}
                  </h2>
                  <div className={s.header_shapes}>
                    <div className={s.header_counter}>
                      <h2 className={s.counter_text}>
                        {t("patientNewTherapy.pickedSessions")}{" "}
                        {selectedSessions.length}/{numOfSessions}
                      </h2>
                    </div>
                    <div className={s.info_duration}>
                      <h2 className={s.duration_text}>
                        {t("patientNewTherapy.duration")} {duration}{" "}
                        {duration == 1
                          ? t("patientNewTherapy.oneDay")
                          : duration > 20 && duration % 10 == 1
                          ? t("patientNewTherapy.croDay")
                ...
                <p className={s.sessions_subtitle}>
                  {t("patientNewTherapy.restrictions")}
                </p>
                <div className={s.sessions_info}>
                  <p className={s.sessions_restrictions}>
                    1.&#160;{" "}
                    {t(
                      "patientNewTherapy.sessionRestrictions.firstRestriction1"
                    )}
                    <span>
                      {t(
                        "patientNewTherapy.sessionRestrictions.firstRestriction2"
                      )}
                    </span>
                    <br /> {/* extra space za poravnanje */}
                    2.{" "}
                    {t(
                      "patientNewTherapy.sessionRestrictions.secondRestriction1"
                    )}
                    <span>
                      {numOfSessions * 5}{" "}
                      {t(
                        "patientNewTherapy.sessionRestrictions.secondRestriction2"
                      )}
                    </span>
                    <br />
                    3.{" "}
                    {t(
                      "patientNewTherapy.sessionRestrictions.thirdRestriction1"
                    )}
                    <span>
                      {t(
                        "patientNewTherapy.sessionRestrictions.thirdRestriction2"
                      )}
                    </span>{" "}
                    {t(
                      "patientNewTherapy.sessionRestrictions.thirdRestriction3"
                    )}
                  </p>
                </div>
                <p className={s.schedule_legend}>
                  {t("patientNewTherapy.scheduleLegend1")}&#160;
                  <br className={s.mobile_only} />
                  {t("patientNewTherapy.scheduleLegend2")}{" "}
                  <span className={s.legend_purple}>
                    {t("patientNewTherapy.scheduleLegend3")}
                  </span>
                  <br />
                </p>
            ...
            {progress == 3 && (
              <>
                ...
                <div className={s.final_main}>
                  <div className={s.final_review}>
                    ...
                    <div className={s.review_card}>
                      ...
                      <div className={s.review_details}>
                        ...
                        <p>
                          {t("patientNewTherapy.durationDays1")} {duration}{" "}
                          {t("patientNewTherapy.durationDays2")}
                        </p>
                        <p>
                          {t("patientNewTherapy.noOfSessions")}{" "}
                          {selectedTherapy.numOfSessions}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div
                    className={`${s.final_sessions} ${
                      expandSessions && s.sessions_expanded
                    }`}
                  >
                    <div className={s.sessions_card}>
                      <h3 className={s.card_title}>
                        {t("patientNewTherapy.mySessions")}
                      </h3>
                      <div className={s.sessions_container}>
                        {sessionElements}
                      </div>
                      <p
                        className={s.sessions_expand}
                        onClick={() =>
                          setExpandSessions((prevState) => !prevState)
                        }
                      >
                        {expandSessions
                          ? t("patientNewTherapy.collapse")
                          : t("patientNewTherapy.expand")}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className={s.final_verification}>
                    ...
                    <form className={s.verification_form} autoComplete="off">
                      <div className={s.verification_input}>
                        ...
                        <div className={s.input_wrapper}>
                          <input
                            ...
                            type="text"
                            placeholder="DGC_123456"
                            name="referral"
                            value={verificationData.referral}
                          />
                          <p
                            className={s.verification_tip}
                            title={tooltips.referral}
                          >
                            ?
                          </p>
                        </div>
                      ...
                      <div className={s.verification_input}>
                        ...
                        <div className={s.input_wrapper}>
                          <input
                            ...
                            name="hlkid"
                            value={verificationData.hlkid}
                          />
                          <p
                            className={s.verification_tip}
                            title={tooltips.doctor}
                          >
                            ?
                          </p>
                        </div>
                      ...
                      <p
                        className={`${s.verification_failed} ${
                          verificationFailed && s.visible
                        }`}
                      >
                ...
                <div className={s.final_finish}>
                  <p className={s.finish_note}>
                    {t("patientNewTherapy.finishNote")}
                  </p>
                  <div
                    className={s.checkbox_container}
                    onClick={() =>
                      setFinishAgreement((prevState) => !prevState)
                    }
                  >
                    <div className={s.custom_checkbox}>
                      <div
                        className={`${s.checkbox_fill} ${
                          finishAgreement && s.checkbox_selected
                        }`}
                      ></div>
                    </div>
                    <p className={s.finish_label}>
                      {t("patientNewTherapy.iUnderstand")}
                    </p>
                  </div>
                </div>
              </>
            ...
            <div className={s.create_buttons}>
              ...
              >
                {progress == 1
                  ? t("patientNewTherapy.cancelButton")
                  : t("patientNewTherapy.backButton")}
              </button>
              <button
                className={`${s.button_next} ${
                  nextDisabled() ? s.button_disabled : ""
                }`}
                onClick={() => {
                  nextDisabled()
                    ? ""
              ...
              >
                {progress == 3
                  ? t("patientNewTherapy.finishButton")
                  : t("patientNewTherapy.nextButton")}
              </button>
            </div>
        ...
        <div className={s.tagline_container}>
          <h1 className={s.tagline}>
            {t("patientNewTherapy.tagline1")}
            <br />
            <span>{t("patientNewTherapy.tagline2")}</span>
          </h1>
        </div>
      </div>
