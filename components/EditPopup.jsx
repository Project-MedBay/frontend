...
export default function AccountEditPopup(props) {
  ...
  const [formData, setFormData] = useState(() => {
    if (popupFor == "therapist" || popupFor == "patient")
      return {
        ...popupData,
        password: "",
        passwordConfirm: "",
  ...
  const [inputFailed, setInputFailed] = useState({
    ...
    "e-mail": {
      failed: false,
      text: "Email must be in format 'something@something.domain'.",
    },
  ...
  const [editingPassword, setEditingPassword] = useState(
    popupType == "add" && popupFor == "therapist"
  );
  ...
  if (popupFor == "therapy") {
    const bodypartList = [
      t("bodyPartSelection.head"),
      t("bodyPartSelection.upper torso"),
      t("bodyPartSelection.lower torso"),
      t("bodyPartSelection.shoulder"),
      t("bodyPartSelection.arm"),
      t("bodyPartSelection.hand"),
      t("bodyPartSelection.leg"),
      t("bodyPartSelection.foot"),
      t("bodyPartSelection.any"),
    ];
        ...
        <div className={s.custom_checkbox}>
          <div
            className={`${s.checkbox_fill} ${
              formData.bodypart == bodypart && s.checkbox_selected
            }`}
          ></div>
        </div>
        <p className={s.checkbox_label}>
    ...
    return (
      <div className={s.form_input} id={s[id]} key={name}>
        {name == "passwordConfirm" && !editingPassword ? (
          <>
            <p className={s.change_password_note}>
              &#40;{t("adminEditPopup.changePasswordNote")}&#41;
            </p>
            <button
              className={s.change_password}
              onClick={(event) => handlePassEdit(event)}
            >
          ...
          <>
            <p className={s.input_label}>
              {t("formFields." + popupFor + "." + name + ".label")}{" "}
              {popupType == "edit" &&
                name == "passwordConfirm" &&
                editingPassword && (
                  <span onClick={cancelPassEdit}>
                    {t("adminEditPopup.cancelEdit")}
                  </span>
                )}
            </p>
            {nonEditableFields.includes(name) ? (
              ...
              <textarea
                ...
                className={`${s.desc_box} ${
                  inputFailed[name].failed && s.failed_input
                }`}
                type="text"
                onChange={handleChange}
                placeholder={t(
                  "formFields." + popupFor + "." + name + ".placeholder"
                )}
                name={name}
                value={formData[name]}
              />
  ...
  function checkEmployedSinceRules(value) {
    // provjera za datum zaposlenja (trenutno format YYYY-MM-DD:, mora biti moguc datum)
  ...
  function handleSave() {
    ...
    if (
      ...
      // blokiraj slanje ako triba unit sifru a nije tocna
      checkPasswordCorrect().then((res) => {
        if (res) {
  ...
  async function checkPasswordCorrect() {
    ...
    let correct = () =>
      axios({
        url:
          "https://medbay-backend-4957d331fef0.herokuapp.com/api/user/check-password?id=" +
          (popupFor == "patient" ? "0" : popupData.id) +
          "&password=" +
          formData.password,
        method: "GET",
        headers: {
    ...
    if (correct().then((res) => console.log(res)))
      setInputFailed((prevInputFailed) => ({
        ...prevInputFailed,
  ...
  return (
    <div className={`${s.edit_popup} ${darkModeClass}`}>
      <h1 className={s.popup_title}>
        {popupType == "add"
          ? t("adminEditPopup.titleAdd")
          : t("adminEditPopup.titleEdit")}
        &#160;
        {popupFor == "patient"
          ? t("adminEditPopup.titleAccountData")
          : t("adminEditPopup.title " + popupFor).toUpperCase()}
      </h1>
      ...
      <div className={s.popup_buttons}>
        <button className={s.button_cancel} onClick={popupExit}>
          {t("adminEditPopup.buttonCancel")}
        </button>
        <button className={s.button_save} onClick={handleSave}>
          {t("adminEditPopup.buttonSave")}
        </button>
      </div>
    </div>
  );
