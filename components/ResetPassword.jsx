...
export default function ResetPassword(props) {
  ...
  function handleSubmit(event) {
    ...
    if (formData.password.length < 8)
      ...
      axios({
        url:
          "https://medbay-backend-4957d331fef0.herokuapp.com/api/security/change-password?token=" +
          resetToken +
          "&password=" +
          formData.password,
        method: "PUT",
  ...
  return (
    <>
      <form
        className={s.reset_password_form}
        onSubmit={handleSubmit}
        autoComplete="off"
      >
        <h1 className={s.form_title}>{t("resetPassword.resetTitle")}</h1>
        <p
          className={`${s.reset_failed} ${inputFailed.value && s.failed_text}`}
        >
          {inputFailed.text}
        </p>
        ...
        {!success ? (
          ...
          <>
            ...
            <h3 className={s.login} onClick={() => globalNavigate("/login")}>
              {t("resetPassword.takeToLogin")}
            </h3>
          </>
        )}
      </form>
