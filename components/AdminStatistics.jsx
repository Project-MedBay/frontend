...
export default function AdminStatistics(props) {
  ...
  useEffect(() => {
    axios({
      url: "https://medbay-backend-4957d331fef0.herokuapp.com/api/employee/statistics",
      method: "GET",
      headers: {
      ...
      .then((res) => {
        ...
        for (let therapist in res.data.percentageOfHoursWorkedLastMonth) {
          ...
          percentAverage +=
            res.data.percentageOfHoursWorkedLastMonth[therapist];
        }
        ...
        let patientsList = [];
        for (let therapist in res.data.numberOfSessions)
          patientsList.push({
            name: therapist,
    ...
    axios({
      url: "https://medbay-backend-4957d331fef0.herokuapp.com/api/equipment/statistics",
      method: "GET",
      headers: {
        Authorization: `Bearer ${userToken}`, // korisnikov access token potreban za dohvacanje podataka iz baze
      },
  ...
  const smallCircles = pageInfo.hours
    .slice(1)
    ...
    .map((item) => (
      <div className={s.one_circle_container}>
        <AdminStatisticsCircle percentage={item.percentage} size="small" />
        <div className={s.circle_name}>{item.name}</div>
      </div>
    ));
    ...
    .map((item) => (
      <div className={s.bar_item}>
        ...
        <div className={s.coloured_bar_wrapper}>
          <div
            className={s.coloured_bar}
            style={{ width: `${100 * (item.noOfPatients / bestItem)}%` }}
          >
            ­
          </div>
        </div>
  ...
  return (
    <div className={s.admin_stats_main}>
      <div className={s.stats_two_options}>
        <h2
          className={`${s.options_item} ${
            currentStatistic == "therapists" ? s.current_option : ""
          }`}
          onClick={() => {
        ...
        <h2
          className={`${s.options_item} ${
            currentStatistic == "resources" ? s.current_option : ""
          }`}
          onClick={() => {
      ...
      <input
        ...
        type="text"
        onChange={(event) => handleSearch(event.target.value, "therapists")}
        placeholder={t("adminStatistics.placeholder")}
        name="search"
        value={searchInput[currentStatistic]}
        autoComplete="off"
      />
      <div className={s.stats_container}>
        <div className={s.container_left}>
          <h2 className={s.container_main_text}>
            {t("adminStatistics.workHours")}{" "}
            {currentStatistic == "therapists"
              ? t("adminStatistics.appointments")
              : t("adminStatistics.use")}
          </h2>
          <h4 className={s.container_secondary_text}>
            {t("adminStatistics.pastMonth")}
          </h4>
          ...
          <div className={s.left_small_circles}>{smallCircles}</div>
        </div>
        <div className={s.container_right}>
          <h2 className={s.container_main_text}>
            {t("adminStatistics.patientsTreated")}{" "}
            {currentStatistic == "therapists"
              ? t("adminStatistics.by")
              : t("adminStatistics.with")}
          </h2>
          <h4 className={s.container_secondary_text}>
            {t("adminStatistics.lifetime")}
          </h4>
          <div className={s.right_bar_container}>{noOfPatients}</div>
        </div>
      </div>
    </div>
  );
