...
export default function LoginMain(props) {
  ...
  const [inputFailed, setInputFailed] = useState({
    // state za gresku pri unosu forme (on submit)
    value: false,
  ...
  function handleSubmit(event) {
    ...
    axios({
      url: "https://medbay-backend-4957d331fef0.herokuapp.com/api/security/login",
      method: "POST",
      data: formData,
    })
  ...
  function handleError(error) {
    // ispisuje error u konzoli, ako je error code 403 (bad request - forbidden) znaci da baza nije dopustila unos
    ...
    if (error.response.status == 403)
      ...
      setInputFailed({
        value: true,
        text: `${error.message}` + t("login.tryAgain"),
      });
  }
  ...
  return (
    <>
      <div
        className={`${s.login_main} ${forgotPassword && s.covered_by_popup}`}
      >
        <form
        ...
        >
          ...
          <p
            className={`${s.login_failed} ${
              inputFailed.value && s.failed_text
            }`}
          >
            {" "}
            {/* uvjetno dodajemo klasu koja cini poruku o gresci vidljivom */}
            {inputFailed.text}
          </p>

          <div className={s.form_input}>
            {" "}
            {/* prvo od dva input polja, takoder imaju uvjetnu klasu za gresku, tekst polja odgovara sadrzaju statea formData */}
            <p className={s.input_text}>{t("login.emailLabel")}</p>
            <input
              className={`${s.input_box} ${
                inputFailed.value && s.failed_input
              }`}
              type="text"
          ...
          <div className={s.form_input}>
            ...
            <div className={s.password_container}>
              ...
              <img
                src={
                  passwordShown ? eyeShown : eyeHidden
                } /* uvjetni izbor slike oka za toggle lozinke */
          ...
          <div
            className={s.forgot_password_div}
            onClick={() => setForgotPassword(true)}
          >
            <h4 className={s.forgot_password_text}>
        ...
        <div className={s.extras}>
          ...
          <div className={s.register_container}>
            ...
            >
              ...
              {/* uporaba prop funkcije za navigaciju */}
            </button>
          </div>
        </div>
      </div>
