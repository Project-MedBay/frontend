...
export default function AdminManage(props) {
  ...
  const selectData = () => {
    if (popupFor() == "therapist" || popupFor() == "resource")
      return specializationList.map((specialization) => ({
        value: specialization,
        label:
          specialization[0] +
          specialization.toLowerCase().split("_").join(" ").slice(1),
      }));
  ...
  useEffect(() => {
    axios({
      url: "https://medbay-backend-4957d331fef0.herokuapp.com/api/equipment/facility",
      method: "GET",
      headers: {
  ...
  const resourceElements = resourceList
    .filter((resource) => {
      for (let term of searchInput.resources.trim().split(" ")) {
        for (let attr in resource) {
          if (attr == "id" || attr == "description") continue;
            ...
            resource[attr].toString().toLowerCase().includes(term.toLowerCase())
          )
            return true;
        }
      }
  ...
  if (resourceElements.length == 0) {
    resourceElements.push(
      <h3 className={s.no_results}>{t("adminManage.noItemsMatchQuery")}</h3>
    );
  }

  const therapyElements = therapyList
    .filter((therapy) => {
      for (let term of searchInput.therapies.trim().split(" ")) {
        for (let attr in therapy) {
          if (attr == "id" || attr == "description") continue;
            ...
            therapy[attr].toString().toLowerCase().includes(term.toLowerCase())
          )
            return true;
        }
      }
  ...
  function handleAdd(subject) {
    if (!addPopup) {
      ...
      axios({
        url:
          "https://medbay-backend-4957d331fef0.herokuapp.com/api/" + endpoint,
        method: "POST",
        headers: {
  ...
  function handleEdit(subject) {
    if (!editPopup) {
      if (subject["employed since"] != null)
        setPopupData({
          ...subject,
          specialization: {
  ...
  return (
    <>
      <div
        className={`${s.admin_manage_main} ${
          (addPopup || editPopup || deactivatePopup) && s.covered_by_popup
        }`}
      >
        <div className={s.three_options}>
          <h2
            className={`${s.options_item} ${
              currentManage == "therapists" ? s.current_option : ""
            }`}
            id={s.left}
            onClick={() => setCurrentManage("therapists")}
          >
            {t("adminManage.therapistsTab")}
          </h2>
          <h2
            className={`${s.options_item} ${
              currentManage == "patients" ? s.current_option : ""
            }`}
            id={s.middle}
            onClick={() => setCurrentManage("patients")}
          >
            {t("adminManage.patientsTab")}
          </h2>
          <h2
            className={`${s.options_item} ${
              currentManage == "facility" ? s.current_option : ""
            }`}
            id={s.right}
            onClick={() => setCurrentManage("facility")}
          >
            {t("adminManage.facilityTab")}
          </h2>
        </div>
        ...
        {currentManage == "therapists" && (
          <>
            <input
              ...
              type="text"
              onChange={(event) =>
                handleSearch(event.target.value, "therapists")
              }
              placeholder={t("adminManage.searchPlaceholder")}
              name="search"
              value={searchInput.therapists}
              autoComplete="off"
            />
        ...
        {currentManage == "patients" && (
          <>
            <input
              className={s.form_search}
              type="text"
              onChange={(event) => handleSearch(event.target.value, "patients")}
              placeholder={t("adminManage.searchPlaceholder")}
              name="search"
              value={searchInput.patients}
              autoComplete="off"
            />
            ...
            <div className={s.manage_facility}>
              <div className={s.facility_section}>
                <input
                  ...
                  type="text"
                  onChange={(event) =>
                    handleSearch(event.target.value, "resources")
                  }
                  placeholder={t("adminManage.searchPlaceholder")}
                  name="search"
                  value={searchInput.resources}
                  autoComplete="off"
                />
                <h1 className={s.section_title}>
                  {t("adminManage.resourcesSectionTitle")}
                </h1>
              ...
              <div className={s.facility_section}>
                <input
                  ...
                  type="text"
                  onChange={(event) =>
                    handleSearch(event.target.value, "therapies")
                  }
                  placeholder={t("adminManage.searchPlaceholder")}
                  name="search"
                  value={searchInput.therapies}
                  autoComplete="off"
                />
                <h1 className={s.section_title}>
                  {t("adminManage.therapiesSectionTitle")}
                </h1>
                <div className={s.card_container}>
                  {therapyElements}
                  <button
                    className={s.section_button}
                    onClick={() => handleAdd("therapy")}
                  >
                    {t("adminManage.addNewButton")}
                  </button>
                </div>
      ...
      {(addPopup || editPopup || deactivatePopup) && (
